addEventHandler('onClientResourceStart', resourceRoot,
    function()
		...
		local txd = engineLoadTXD('stopsign.txd',true)
        engineImportTXD(txd, 1352  )
        local dff = engineLoadDFF('stopsign.dff', 1352  )
        engineReplaceModel(dff, 1352  )
		
function ReplaceCollision ( )
col = engineLoadCOL( "stopsign.col" )
engineReplaceCOL( col, 1352 )
end
...
addEventHandler ( "collisionReplace", root, ReplaceCollision ) 
        ...
        engineImportTXD(txd, 11453  )
		 ...
		 local txd = engineLoadTXD('sw_med1.txd', 12983  )
        engineImportTXD(txd, 12983  )
