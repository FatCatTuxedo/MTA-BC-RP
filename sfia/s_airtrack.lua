...
function togRadar(thePlayer, commandName, ...)
	local theTeam = getPlayerTeam(thePlayer)

	if (theTeam) then
		local teamID = tonumber(getElementData(theTeam, "id"))
		if (teamID==83) then
			exclude[thePlayer] = true
			off( thePlayer )
			triggerClientEvent(thePlayer, "sfia:airtrack:blips", thePlayer, blips)
		end
	end
end
addCommandHandler("airportradar", togRadar)
