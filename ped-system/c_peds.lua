...
function selectPedTypeWindow()
	...
	local possiblePedTypes = {
		...
		{"clothing", "Clothing Ped", 4},
		{"guard", "Guard Ped", 1},
		
		{"fuel", "Fuel Station Ped", 4},
		{"bank.banking", "Bank Ped", 4},
		{"bank.atmcard", "Bank ATM Ped", 4},
		...
		{"dmv.transfer", "DMV: Transfer Paper Ped", 4},
		{"dmv.lost", "DMV: Lost License Ped", 4},

		{"sfes.reception", "ES Reception Ped", 4},
		{"astro.guard", "Astro Guard Ped", 4},
		{"astro.pay", "Astro Enter Guard Ped", 4},
		{"towing.release", "LSCM Release Ped", 4},
		{"make.generic", "RS Haul Make Generic", 4},
	}
...
function selectPedType(button, state)
	...
	if(selectedRow >= 0) then
		...
		elseif(selectedType == "clothing") then --clothing ped
			guiSetText(GUIEditor.edit[10], "[auto]") --set auto name
			guiSetText(GUIEditor.edit[3], "211") --set skin
			guiComboBoxSetSelected(GUIEditor.combo[1], GUIEditor.comboItem[1]) --set frozen true
		end
	end
end
