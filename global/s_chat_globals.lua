...
function sendMessageToAdmins(message,showToOffDutyAdmins)
	...
	for k, thePlayer in ipairs(players) do
		if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
			if showToOffDutyAdmins or ((getElementData(thePlayer, "duty_admin") == 1) or (getElementData(thePlayer, "duty_dev") == 1)) then
...
function sendMessageToStaff(message,showToOffDutyStaff)
	...
	for k, thePlayer in ipairs(players) do
		if exports.integration:isPlayerStaff(thePlayer) then
			if showToOffDutyStaff or getElementData(thePlayer, "duty_dev") == 1 or getElementData(thePlayer, "duty_admin") == 1 or getElementData(thePlayer, "duty_supporter") == 1 then
				outputChatBox(message, thePlayer, 255, 0, 0)
			end
		end
	end
