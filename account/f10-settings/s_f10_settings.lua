...
function loadAccountSettings(player,id)
	if player and id then
		...
		while true do
			...
			if (row.name == 'duty_dev' and not exports.integration:isPlayerScripter(player)) or (row.name == 'duty_admin' and not exports.integration:isPlayerTrialAdmin(player)) or (row.name == 'duty_supporter' and not exports.integration:isPlayerSupporter(player)) then
				row.value = 0
			end
...
function saveClientAccountSettingsOnServer(name, value)
	...
	if name == "duty_admin" or name == "duty_supporter" or name == "duty_dev" then
