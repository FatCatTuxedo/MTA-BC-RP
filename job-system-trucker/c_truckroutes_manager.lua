...
function clearAllMarkers()
	if exports.global:isPlayerFullAdmin(getLocalPlayer()) then
		local count = 0
...
function delTruckerMarker( cm, id) 
	...
	if not exports.global:isPlayerFullAdmin(sourcePlayer) then
		outputChatBox("Only Full Admins and above can access /"..cm..".", 255,0 ,0 )
		return false  
	end     
	
	if not tonumber(id) or not (tonumber(id) >= 0) then
		outputChatBox("SYNTAX: /" .. cm .. " [Route ID]", 255, 194, 14)
		return false
	end
