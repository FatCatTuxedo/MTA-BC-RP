...
function showFactionMenu(motd, memberUsernames, memberRanks, memberPerks, memberLeaders, memberOnline, memberLastLogin, --[[memberLocation,]] factionRanks, factionWages, factionTheTeam, note, fnote, vehicleIDs, vehicleModels, vehiclePlates, vehicleLocations, intIDs, intNames, memberOnDuty, towstats, phone, membersPhone, fromShowF, factionID)
	if (gFactionWindow==nil) then
		...
		if (localPlayerIsLeader) then
			...
			tabInts = guiCreateTab("(Leader) Interiors", tabs)
		
			gIntGrid = guiCreateGridList(0.01, 0.015, 0.8, 0.905, true, tabInts)
		
			colIntID = guiGridListAddColumn(gIntGrid, "ID", 0.2)
			colIntName = guiGridListAddColumn(gIntGrid, "Name", 0.7)
			
			for index, intID in ipairs(intIDs) do
				local rowi = guiGridListAddRow(gIntGrid)
				guiGridListSetItemText(gIntGrid, rowi, colIntID, tostring(intID), false, true)
				guiGridListSetItemText(gIntGrid, rowi, colIntName, tostring(intNames[index]), false, false)
			end
