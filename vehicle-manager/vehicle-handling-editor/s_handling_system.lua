...
function createDealerNPCs(which)
	...
	if which == getThisResource() or which == "Christopher Jackson" then
		...
		dealershipPed[1] = createPed( 240, 175.6689453125, 1188.7529296875, 14.7578125 )
		setPedRotation( dealershipPed[1], 177 )
...
--[[	if which == getThisResource() or which == "Joseph Moore" then
		--outputDebugString("Created")
		dealershipPed[3] = createPed( 28, 30.3818359375, 1218.9658203125, 19.224761962891 )
	...
	if which == getThisResource() or which == "Brandon Harris" then
		--outputDebugString("Created")
		dealershipPed[4] = createPed( 36, 182.9140625, 1171.67578125, 14.7578125 )
		setPedRotation( dealershipPed[4], 58.10162353516 )
	...
	if which == getThisResource() or which == "Jacob Jones" then
		--outputDebugString("Created")
		dealershipPed[5] = createPed( 37, 164.0849609375, 1187.958984375, 14.7578125 )
		setPedRotation( dealershipPed[5], 181 )
	...
	if which == getThisResource() or which == "John Garcia" then
		--outputDebugString("Created")
		dealershipPed[9] = createPed( 16, 162.3779296875, 1181.1640625, 14.818670272827 )
		setPedRotation( dealershipPed[9], 267 )
...
<b>Edited by:   </b>]=] ..tostring(getElementData(who, "account:username")) ..[=[<br>
]=]

	if handling then
		forumText = forumText .. "<b>[size=5]New Handling:[/size]</b>\n"
		...
		for _, x in ipairs({handlingValues, {{"Drive Type", ref = 'driveType'}}}) do
			for _, v in ipairs(x) do
				...
				forumText = forumText .. "<b>" .. v[1] .. ":</b>   " .. (type(h) == 'table' and table.concat(h, ',') or type(h) == 'number' and ("%.2f"):format(h) or tostring(h)) .. "<br>"
			end
		end
	end
