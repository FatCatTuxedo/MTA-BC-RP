...
function showLibrary(vehs, thePed)
	...
	GUIEditor_Window[1] = guiCreateWindow((sx-w)/2,(sy-h)/2,w,h,"Vehicles System",false)
			...
			function( button )
				if button == "left" then
					...
					if row ~= -1 and col ~= -1 then
						...
						if createdby ~= getElementData(localPlayer, "account:username") and not exports.integration:isPlayerLeadAdmin(localPlayer)  then
