...
function sendWarningToCops(theVehicle, thePlayer, speed, x, y, z)
	...
	if tonumber(getElementData(theVehicle, "faction")) == 1 or tonumber(getElementData(theVehicle, "faction") == 59) or tonumber(getElementData(theVehicle, "dbid") == 331) or tonumber(getElementData(theVehicle, "faction")) == 2 or getElementModel(theVehicle) == 481 or getElementModel(theVehicle) == 509 or getElementModel(theVehicle) == 510 then
		return
	else
		local teamPlayers = { }
		local PDmembers = getTeamFromName("Bone County Sheriff's Office")
		local SDmembers = getTeamFromName("Federal Bureau of Investigation")
