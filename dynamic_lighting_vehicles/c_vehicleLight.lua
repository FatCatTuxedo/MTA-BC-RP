...
local flTimerUpdate = 200 -- default 200, the effect update time interval
local light_states = {
	[1] = {
...
function renderVehicleLighting()
	...
	for dbid, this in pairs( getElementsByType('vehicle') ) do
...
function startVehicleLighting()
	...
	FLenTimer = setTimer( function()
		for dbid, thisVeh in pairs( getElementsByType('vehicle') ) do
			if thisVeh and isElement(thisVeh) and isElementStreamedIn(thisVeh) then
