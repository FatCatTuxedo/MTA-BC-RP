...
function openStaffManager(staffInfo1, staffTeams1, globalChangelogs1) --/ MAXIME
	if canPlayerAccessStaffManager(localPlayer) then
		...
		if GUIEditor.window[1] and isElement(GUIEditor.window[1]) then
	        ...
	        GUIEditor.window[1] = guiCreateWindow(538, 144, 700, 461+yExtend, "Staff Manager", false)
	        ...
	        GUIEditor.tab.home = guiCreateTab("Edit Staff Member", GUIEditor.tabpanel[1])
	        ...
	        GUIEditor.label[4] = guiCreateLabel(10, 101, 74, 22, "Helper:", false, GUIEditor.tab.home)
	        ...
	        GUIEditor.label[5] = guiCreateLabel(10, 133, 74, 22, "Other:", false, GUIEditor.tab.home)
	        ...
	        GUIEditor.label[6] = guiCreateLabel(10, 165, 74, 22, "Developer:", false, GUIEditor.tab.home)
...
function onTabSwitch( selectedTab )
	if GUIEditor.window[1] and isElement(GUIEditor.window[1]) then
		...
		if tab == "Admins" or tab == "Helpers" or tab == "Other" or tab == "Developers" then 
...
function guiStaffChange()
	...
	elseif source == GUIEditor.combobox.sup then
		...
		if (not exports.integration:isPlayerSupportManager(localPlayer)) and not exports.integration:isPlayerLeadAdmin(localPlayer) then
	...
	elseif source == GUIEditor.combobox.vct then
		...
		if not exports.integration:isPlayerLeadAdmin(localPlayer) then
