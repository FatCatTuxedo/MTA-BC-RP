...
function openLocationManager(locationData)
    ...
    gui.manager = guiCreateWindow(0, 0, w, h, "/places Manager", false)
    ...
    for v, k in pairs(data) do
        ...
        guiGridListSetItemText(a, b, gui.colID, v, false, true)
        guiGridListSetItemText(a, b, gui.colValue, string.upper(k[8]), false, false)
        guiGridListSetItemText(a, b, gui.colDesc, k[6], false, false)
        guiGridListSetItemText(a, b, gui.colCreator, k[7], false, false)
    end
    ...
    addEventHandler("onClientGUIClick", gui.deleteSelected, function ()
            ...
            if r ~= -1 and c ~= -1 then
                ...
                triggerServerEvent("server:deleteLocation", localPlayer, localPlayer, id)
                ...
                if source == gui.newCreate then --1
                    ...
                    if string.len(newValue) < r or string.len(newDesc) < r then
						...
						elseif string.find(newValue, " ") then
                            ...
                            triggerServerEvent("server:addNewLocation", localPlayer, localPlayer, newValue, newDesc)
                            exports.global:playSoundCreate()
                            updateLocations()
                            terminateNewLocation()
...
function updateLocations()
    triggerServerEvent("server:loadPlaces", root, localPlayer)
end
