...
function useItem(itemSlot, additional)
	...
	if itemID then
		if (itemID==215) then -- lightbar
			local vehicle = getPedOccupiedVehicle(source)
			if (vehicle) then
				exports.global:sendLocalMeAction(source, "adds an Emergancy Light Bar to their " .. exports.global:getVehicleName(vehicle).."("..getVehicleName(vehicle) .. ").")
				moveItem(source, vehicle, itemSlot)
				triggerEvent("police:addLightBar", vehicle)
			end
		elseif (itemID==1) then -- hotdog
			setElementHealth(source, 100)
			triggerEvent('sendAme', source, "eats a hotdog.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==216) then -- cinema remote
			triggerClientEvent(source, "cinema:showGUI", source)
		...
		elseif masks[itemID] then
			...
			if mask then
				...
				elseif itemID == 217 then -- Full Face Helmet
			...
			elseif getPedOccupiedVehicle(source) and getElementData(getPedOccupiedVehicle(source), "job") > 0 and getVehicleOccupant(getPedOccupiedVehicle(source)) == source then
				...
				elseif itemID == 217 and not (getElementModel(source) == 285) and not (getElementModel(source) == 75) then -- swat
					local customTexture = getItemTexture(itemID, itemValue)
					triggerEvent("artifacts:add", source, source, "kevlar", false, customTexture)
		...
		elseif (itemID==172) then
			outputChatBox("Only motocross helmets work heres one for free.", source, 0, 255, 0)
			takeItemFromSlot(source, itemSlot)
...
function toggleBadge(source, badges, itemID , itemValue)
	if ( getElementData( source, badges[itemID][1] ) ) then
		exports.anticheat:changeProtectedElementDataEx(source, badges[itemID][1], false, true)
		if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 or itemID == 158 or itemID == 168  or itemID == 222 then
		...
		elseif itemID == 222 then
			exports.anticheat:changeProtectedElementDataEx( source, badges[itemID][1], true )
			triggerEvent('sendAme',  source, "puts a " .. badges[itemID][1] .. " onto their head." )
		else
