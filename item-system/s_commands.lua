...
function givePlayerFBIBadge(thePlayer, commandName, targetPlayer, ... )
	local badgeNumber = table.concat( { ... }, " " )
	local theTeam = getPlayerTeam(thePlayer)
	local teamID = getElementData(theTeam, "id")
	...
	if not (tonumber(leader)==1 and teamID == 80) then -- If the player is not the leader
		outputChatBox("You must be a faction leader to issue badges.", thePlayer, 255, 0, 0) -- If they aren't leader they can't give out badges.
	else
		if not targetPlayer or not badgeNumber then
			outputChatBox("SYNTAX: /" .. commandName .. " [Player Partial Nick / ID] [Badge Number]", thePlayer, 255, 194, 14)
			...
			if targetPlayer then -- is the player online?
				...
				if (logged==0) then -- Are they logged in?
					...
					if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)>4) then -- Are they standing next to each other?
						...
						exports.global:sendLocalMeAction(thePlayer, "issues "..targetPlayerName.." an FBI Badge, reading " .. badgeNumber .. ".")
						exports.global:giveItem(targetPlayer, itemID, badgeNumber)
					end
				end
			end
		end
...
addCommandHandler("issuefbibadge", givePlayerFBIBadge, false, false)

function givePlayerFDBadge(thePlayer, commandName, targetPlayer, ... )
	local badgeNumber = table.concat( { ... }, " " )
	local theTeam = getPlayerTeam(thePlayer)
	local teamID = getElementData(theTeam, "id")
	...
	if not (tonumber(leader)==1 and teamID == 1) then -- If the player is not the leader
		outputChatBox("You must be a faction leader to issue badges.", thePlayer, 255, 0, 0) -- If they aren't leader they can't give out badges.
	else
		if not targetPlayer or not badgeNumber then
			outputChatBox("SYNTAX: /" .. commandName .. " [Player Partial Nick / ID] [Badge Number]", thePlayer, 255, 194, 14)
			...
			if targetPlayer then -- is the player online?
				...
				if (logged==0) then -- Are they logged in?
					...
					if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)>4) then -- Are they standing next to each other?
						...
						exports.global:sendLocalMeAction(thePlayer, "issues "..targetPlayerName.." a BCVFD badge, reading " .. badgeNumber .. ".")
						exports.global:giveItem(targetPlayer, itemID, badgeNumber)
					end
				end
			end
...
addCommandHandler("issuefdbadge", givePlayerFDBadge, false, false)
