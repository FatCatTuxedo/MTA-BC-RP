...
function getItemName(id, value)
	...
	elseif ( id == 217 ) and value then
		local itemValue = explode(";", value)
		if itemValue[1] and itemValue[2] then
			local customName = tostring(itemValue[1]).." tactical vest"
			return customName
		else
			return g_items[id][1]
		end
	...
	elseif (id == 115 or id == 116) and value then
		...
		if (itemExploded[3] == "Deagle") then
			itemExploded[3] = "SIG P220"
		end
		if (itemExploded[3] == "Silenced") then
			itemExploded[3] = "Glock"
		end
		if (itemExploded[3] == "MP5") then
			itemExploded[3] = "AR-15"
		end
...
function getItemTexture( itemID, itemValue )
	if itemID == 90 or itemID == 171 or itemID == 172 or itemID == 217 then --helmet
		if itemValue then
			local texname = {
				[90] = "helmet",
				[171] = "helmet_b",
...
function getPlayerMaxCarryWeight(element)
	...
	if hasItem(element, 217) then weightCount = weightCount + 20 end -- medical bag
	return weightCount
end
