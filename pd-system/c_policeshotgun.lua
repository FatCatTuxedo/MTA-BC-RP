...
function dogAttack(attacker, weapon, bodypart, loss)
	if (weapon == 0 and getElementModel(attacker) == 75) then
		triggerServerEvent("sendAme", attacker, "bites "..getPlayerName(localPlayer):gsub("_", " "))
		triggerServerEvent("sendAme", localPlayer, "falls to the ground after being bitten")
		triggerServerEvent("dogBite", localPlayer, localPlayer)
	end
end
addEventHandler("onClientPlayerDamage", localPlayer, dogAttack)
