				...
				if not (veh) then
					...
					if (insertid) then
						...
						if (hiddenAdmin==0) then
							exports.global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " has spawned a "..vehicleName .. " (ID #" .. insertid .. ") to "..owner.." for $"..cost..".")
						...
						outputChatBox("Please reply to http://forums.ProjectReality.cf/index.php?forums/logs/ with any information you may need to add.", thePlayer, 255, 0, 0)
						if factionVehicle == -1 then
...
-- /makecivveh
function createCivPermVehicle(thePlayer, commandName, ...)
	if exports.integration:isPlayerAdmin(thePlayer) or exports.integration:isPlayerLeadScripter(thePlayer) or exports.integration:isPlayerVehicleConsultant(thePlayer) then
		local args = {...}
		if (#args < 2) then
				outputChatBox("SYNTAX: /" .. commandName .. " [vehlib ID] [Job ID -1 for none]", thePlayer, 255, 194, 14)
				...
				outputChatBox("Job 7 = Miner", thePlayer, 255, 194, 14)
		else

			local vehShopData = exports["vehicle-manager"]:getInfoFromVehShopID(tonumber(args[1]))
			if not vehShopData then
				outputDebugString("VEHICLE SYSTEM / createCivPermVehicle / FAILED TO FETCH VEHSHOP DATA")
				outputChatBox("SYNTAX: /" .. commandName .. " [vehlib ID] [Job ID -1 for none]", thePlayer, 255, 194, 14)
				outputChatBox("Job 1 = Delivery Driver", thePlayer, 255, 194, 14)
				outputChatBox("Job 2 = Taxi Driver", thePlayer, 255, 194, 14)
				outputChatBox("Job 3 = Bus Driver", thePlayer, 255, 194, 14)
			...
			job = tonumber(args[2])
			if (job < -1) or (job > 3 and job ~= 7) then
				outputChatBox("SYNTAX: /" .. commandName .. " [vehlib ID] [Job ID -1 for none]", thePlayer, 255, 194, 14)
				outputChatBox("Job 1 = Delivery Driver", thePlayer, 255, 194, 14)
				outputChatBox("Job 2 = Taxi Driver", thePlayer, 255, 194, 14)
				outputChatBox("Job 3 = Bus Driver", thePlayer, 255, 194, 14)
			...
			y = y + ( ( math.sin ( math.rad ( r ) ) ) * 5 )
				...
				if not (veh) then
					...
					local var1, var2 = exports['vehicle-system']:getRandomVariant(id)
					local smallestID = SmallestID()
					local insertid = mysql:query_insert_free("INSERT INTO vehicles SET id='" .. mysql:escape_string(smallestID) .. "', model='" .. mysql:escape_string(id) .. "', x='" .. mysql:escape_string(x) .. "', y='" .. mysql:escape_string(y) .. "', z='" .. mysql:escape_string(z) .. "', rotx='0', roty='0', rotz='" .. mysql:escape_string(r) .. "', color1='" .. mysql:escape_string(color1) .. "', color2='" .. mysql:escape_string(color2) .. "', color3='" .. mysql:escape_string(color3) .. "', color4='" .. mysql:escape_string(color4) .. "', faction='-1', owner='0', job='" .. mysql:escape_string(job) .. "', plate='" .. mysql:escape_string(plate) .. "', currx='" .. mysql:escape_string(x) .. "', curry='" .. mysql:escape_string(y) .. "', currz='" .. mysql:escape_string(z) .. "', currrx='0', currry='0', currrz='" .. mysql:escape_string(r) .. "', locked=1, interior='" .. mysql:escape_string(interior) .. "', currinterior='" .. mysql:escape_string(interior) .. "', dimension='" .. mysql:escape_string(dimension) .. "', currdimension='" .. mysql:escape_string(dimension) .. "', tintedwindows='0',variant1="..var1..",variant2="..var2..", creationDate=NOW(), createdBy="..getElementData(thePlayer, "account:id")..", `vehicle_shop_id`='"..args[1].."' ")
					if (insertid) then
						reloadVehicle(insertid)

						outputChatBox("[MAKECIVVEH] "..vehicleName .. " (ID #" .. insertid .. ") successfully spawned for Job "..job..".", thePlayer, 0, 255, 0)
...
addCommandHandler("makecivveh", createCivPermVehicle, false, false)
		...
		if veh then
			...
			if tonumber(row.Impounded) > 0 then
				...
				if row.impounder then
					...
					exports.anticheat:changeProtectedElementDataEx(veh, "impounder", 1, false, true) --RT
				end
			end
		...
		if (seat == 0) then
			...
			if not (enginelessVehicle[model]) then
				...
				if engine == 0 then
					...
					if (vehKey) or (owner < 1) and (faction == -1) or (playerFaction == faction) and (faction ~= -1) or ((getElementData(source, "duty_admin") or 0) == 1) then
						...
						if broke == 1 then
							...
							triggerClientEvent(source,"displayMesaage", source, "The engine is broken.", 255, 0, 0)
						...
						triggerClientEvent(source,"displayMesaage", source, "You require a key to start this vehicle.", 255, 0, 0)
					end
				else
			...
			if vehicle and exports['vehicle-interiors']:isNearExit(source, vehicle) then
				...
				--if (owner ~= -1) then
					if ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
						...
						triggerClientEvent(source,"displayMesaage", source, "(( You can't lock impounded vehicles. ))", 255, 195, 14)
					end
				--else
...
function checkBikeLock(thePlayer)
	if (isVehicleLocked(source)) and (getVehicleType(source)=="Bike" or getVehicleType(source)=="Boat" or getVehicleType(source)=="BMX" or getVehicleType(source)=="Quad" or getElementModel(source)==568 or getElementModel(source)==571 or getElementModel(source)==572 or getElementModel(source)==424 or getElementModel(source)==431 or getElementModel(source)==437) then
		if not getElementData(thePlayer, "interiormarker") then
			triggerClientEvent(thePlayer,"displayMesaage", thePlayer, "That vehicle is locked.", 255, 194, 15)
		end
		cancelEvent()
	end
end
...
function setRealInVehicle(thePlayer)
	if isVehicleLocked(source) then
		...
		if owner < 0 and faction == -1 and exports.global:isStaffOnDuty(thePlayer) then
			if brand then
				triggerClientEvent(thePlayer,"displayMesaage", thePlayer, "(( This "..year.." "..brand.." "..model.." is a civilian vehicle. ))", 255, 194, 14)
			else
				triggerClientEvent(thePlayer,"displayMesaage", thePlayer, "(( This "..carName.." is a civilian vehicle. ))", 255, 194, 14)
			end
		elseif (faction==-1) and (owner>0) and exports.global:isStaffOnDuty(thePlayer) then
			local ownerName = exports['cache']:getCharacterName(owner)

			if ownerName then
				if brand then
					triggerClientEvent(thePlayer,"displayMesaage", thePlayer, "(( This "..year.." "..brand.." "..model.." belongs to " .. ownerName .. ". ))", 255, 194, 14)
				else
					triggerClientEvent(thePlayer,"displayMesaage", thePlayer, "(( This "..carName.." belongs to " .. ownerName .. ". ))", 255, 194, 14)
				end
				if (getElementData(source, "Impounded") > 0) then
					local output = getRealTime().yearday-getElementData(source, "Impounded")
					if brand then
						triggerClientEvent(thePlayer,"displayMesaage", thePlayer, "(( This "..year.." "..brand.." "..model.." has been Impounded for: " .. output .. (output == 1 and " Day." or " Days.") .. " ))", 255, 194, 14)
					else
						triggerClientEvent(thePlayer,"displayMesaage", thePlayer, "(( This "..carName.." has been Impounded for: " .. output .. (output == 1 and " Day." or " Days.") .. " ))", 255, 194, 14)
					end
				end
			end
		end
	...
	if (vfaction~=-1) then
		...
		if (faction~=vfaction) and (seat==0) then
			if (CanTowDriverEnter) then
				if brand then
					triggerClientEvent(thePlayer,"displayMesaage", thePlayer, "(( This "..year.." "..brand.." "..model.." belongs to '" .. factionName .. "'. ))", 255, 194, 14)
					
				else
					triggerClientEvent(thePlayer,"displayMesaage", thePlayer, "(( This "..getVehicleName(source).." belongs to '" .. factionName .. "'. ))", 255, 194, 14)
				end
...
function lockUnlockOutside(vehicle)
	if (not source or (exports.integration:isPlayerTrialAdmin(source) and exports.global:isStaffOnDuty(source))) or ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
		local dbid = getElementData(vehicle, "dbid")
		...
		if (isVehicleLocked(vehicle)) then
			...
			triggerEvent('stopCarAlarm', source, vehicle, source)
		else
			setVehicleLocked(vehicle, true)
			for i=0,5 do
				setVehicleDoorOpenRatio ( vehicle, i, 0, 0 )
...
function setVehiclePosition(thePlayer, commandName)
	...
	if not veh or getElementData(thePlayer, "realinvehicle") == 0 then
		...
		elseif isElementWithinColShape( thePlayer, HospitalCol ) and getElementData( thePlayer, "faction" ) ~= 2 and not exports.integration:isPlayerTrialAdmin(thePlayer) and not exports.integration:isPlayerSupporter(thePlayer) then
			outputChatBox("Only the Los SantosFire Department are allowed to park their vehicles in front of the Hospital.", thePlayer, 255, 0, 0)
		elseif isElementWithinColShape( thePlayer, PershingSquareCol ) and getElementData( thePlayer, "faction" ) ~= 1  and not exports.integration:isPlayerTrialAdmin(thePlayer) and not exports.integration:isPlayerSupporter(thePlayer) then
			outputChatBox("Only the Bone County Sheriff's Office are allowed to park their vehicles on Pershing Square.", thePlayer, 255, 0, 0)
		else
...
function setVehiclePosition3(veh)
	if call( getResourceFromName("tow-system"), "cannotVehpos", source ) then
		outputChatBox("Only Los Santos Towing Service is allowed to park their vehicles on the Impound Lot.", source, 255, 0, 0)
	elseif isElementWithinColShape( source, HospitalCol ) and getElementData( source, "faction" ) ~= 2 and not exports.integration:isPlayerTrialAdmin(source) then
		outputChatBox("Only the Los Santos Emergency Service is allowed to park their vehicles in front of the Hospital.", source, 255, 0, 0)
	elseif isElementWithinColShape( source, PershingSquareCol ) and getElementData( source, "faction" ) ~= 1  and not exports.integration:isPlayerTrialAdmin(source) then
		outputChatBox("Only the Los Santos Police is allowed to park their vehicles on Pershing Square.", source, 255, 0, 0)
	else
