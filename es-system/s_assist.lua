...
function removeBackup(thePlayer, commandName)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) or (getElementData(getPlayerTeam(thePlayer), "type") == 4 and getElementData(thePlayer, "factionleader") == 1) then
		if (backupPlayer~=nil) then
			...
			for k,v in ipairs(getPlayersInTeam ( getTeamFromName("Bone County Emergency Services") )) do
			...
			backupPlayer = nil
			backupBlip = false
			outputChatBox("The BCES-Assist system was reset!", thePlayer, 255, 194, 14)
		else
			outputChatBox("The BCES-Assist system did not need a reset.", thePlayer, 255, 194, 14)
		end
	end
end
