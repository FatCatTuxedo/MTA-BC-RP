...
function showIntName(name, ownerName, inttype, cost, ID, bizMsg)
	...
	elseif name then
		if (inttype==3) then -- Interior name and Owner for rented
			...
			if (exports.integration:isPlayerTrialAdmin(getLocalPlayer()) and getElementData(getLocalPlayer(), "duty_admin") == 1) or (exports.integration:isPlayerScripter(getLocalPlayer()) and getElementData(getLocalPlayer(), "duty_dev") == 1) or exports.global:hasItem(getLocalPlayer(), 4, ID) then
			...
			guiLabelSetHorizontalAlign(gInteriorName, "center", true)
			guiSetAlpha(gInteriorName, 0.0)
			if (exports.integration:isPlayerTrialAdmin(getLocalPlayer()) and getElementData(getLocalPlayer(), "duty_dev") == 1) or (exports.integration:isPlayerTrialAdmin(getLocalPlayer()) and getElementData(getLocalPlayer(), "duty_admin") == 1) or exports.global:hasItem(getLocalPlayer(), 4, ID) or exports.global:hasItem(getLocalPlayer(), 5, ID) then
...
function hasKey( key )
	if exports.global:hasItem(getLocalPlayer(), 4, key) or exports.global:hasItem(getLocalPlayer(), 5,key) then
		...
		if getElementData(getLocalPlayer(), "duty_admin") == 1 or getElementData(getLocalPlayer(), "duty_script") == 1 then
			return true, true
		else
			return false, false
		end
...
addEventHandler( "setPlayerInsideInterior2", getRootElement( ),
	function( targetLocation, targetInterior)
		...
		if false and targetInterior then
			local adminnote = tostring(getElementData(targetInterior, "adminnote"))
			if string.sub(tostring(adminnote),1,8) ~= "userdata" and adminnote ~= "\n" and (getElementData(getLocalPlayer(), "duty_admin") == 1 or getElementData(getLocalPlayer(), "duty_script") == 1) then
				outputChatBox("[INT MONITOR]: "..adminnote:gsub("\n", " ").."[..]", 255,0,0)
				outputChatBox("'/checkint "..getElementData(targetInterior, "dbid").." 'for details.",255,255,0)
			end
		end
...
function purchasePropertyGUI(interior, cost, isHouse, isRentable, neighborhood)
	...
	function()
		outputChatBox("Disabled due to bugs.", 255,0,0)
	end, false)
