...
function fixints(thePlayer, commandName)
	if exports.integration:isPlayerAdmin( thePlayer )  then
		local possibleInteriors = exports.pool:getPoolElementsByType('interior')
			for _, interior in ipairs(possibleInteriors) do
				local dbid = getElementData(interior, "dbid")
...
addCommandHandler( "fixints", fixints )
			...
			if exit then
				...
				if query then
					...
					if hiddenAdmin == 0 then
						exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." has changed interior of house #"..dbid.." to ID "..interiorID..".")
					else
		...
		if not cost then
			...
			if exit then
				...
				if query then
					...
					if hiddenAdmin == 0 then
						exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." has changed interior price of house #"..dbid.." to $"..cost..".")
					else
			...
			if exit then
				if type ~= interiorType then
					...
					if query then
						...
						if hiddenAdmin == 0 then
							exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." has changed interior type of house #"..dbid.." to type "..type..".")
						else
		...
		if not id then
			...
			if entrance then
				...
				if interiorStatus[INTERIOR_DISABLED] then
					...
					if hiddenAdmin == 0 then
						exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." has enabled Interior #"..dbid..".")
					else
					...
					if hiddenAdmin == 0 then
						exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." has disabled Interior #"..dbid..".")
					else
			...
			if not houseID then
				...
				if houseID ~= 0 then
					...
					if (query) then
						...
						if hiddenAdmin == 0 then
							exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." has restored Interior #"..houseID..".")
						else
			...
			if not houseID then
				...
				if houseID ~= 0 then
					...
					if query1 and query2 then
						...
						if hiddenAdmin == 0 then
							exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." has removed Interior #"..houseID.." permanently from database.")
						else
	...
	if exports.integration:isPlayerSeniorAdmin( thePlayer ) then	
		if not getElementData(thePlayer, "confirm:removeDeletedInteriors") then
			...
			if mQuery1 then
				...
				local adminUsername = getElementData(thePlayer, "account:username")
				if hiddenAdmin == 0 then
					exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." has executed a massive int remove command on "..count.." deleted interiors permanently from database.", root, 255,0,0)
				else
	...
	if exports.integration:isPlayerSeniorAdmin( thePlayer ) then	
		if not getElementData(thePlayer, "confirm:forceSellAllInactiveInteriors") then
			...
			if mQuery1 then
				...
				local adminUsername = getElementData(thePlayer, "account:username")
				if hiddenAdmin == 0 then
					exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." has executed a massive int force-sell command on "..count.." inactive interiors.")
				else
	...
	if exports.integration:isPlayerSeniorAdmin( thePlayer ) then	
		if not getElementData(thePlayer, "confirm:removeForSaleInteriors") then
			...
			if mQuery1 then
				...
				local adminUsername = getElementData(thePlayer, "account:username")
				if hiddenAdmin == 0 then
					exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." has executed a massive int remove command on "..count.." for-sale interiors permanently from database.", root, 255,0,0)
				else
	...
	if exports.integration:isPlayerSeniorAdmin( thePlayer ) then	
		if not getElementData(thePlayer, "confirm:removeInactiveInteriors") then
			...
			if mQuery1 then
				...
				local adminUsername = getElementData(thePlayer, "account:username")
				if hiddenAdmin == 0 then
					exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." has executed a massive int remove command on "..count.." inactive interiors permanently from database.")
				else
		...
		if not (intID) then
			...
			if entrance then
				...
				if (query) then
					...
					if hiddenAdmin == 0 then
						exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." has moved Interior #"..dbid.." to new location.")
					else
...
addCommandHandler("moveint", updateInteriorEntrance, false, false)
...
function createInterior(thePlayer, commandName, interiorId, inttype, cost, ...)
	if exports.integration:isPlayerAdmin( thePlayer ) then
		...
		elseif not exports.global:takeMoney(getTeamFromName("Fort Carson Municipal Government"), cost) then
			...
			if interior then
				...
				if (query) then
					...
					if hiddenAdmin == 0 then
						exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." has created Interior #"..id.." with name '"..name.."', type "..inttype..", price: $"..cost..").")
					else
...
function changeInteriorName( thePlayer, commandName, ...)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then -- Is the player an admin?
		...
		elseif (dimension==0) then
			...
			if hiddenAdmin == 0 then
				exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." has changed Interior #"..id.."'s name to '"..name.."'.")
			else
