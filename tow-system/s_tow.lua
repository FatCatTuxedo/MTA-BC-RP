...
-- pd db impound lot
local towImpoundSphere = createColCircle (  -112.810546875, 1131.021484375, 5.0 )
...
function UnlockVehicle(element, matchingdimension) 
    if (getElementType(element) == "vehicle" and getVehicleOccupant(element) and getElementData(getVehicleOccupant(element),"faction") == 76 and getElementModel(element) == 525 and getVehicleTowedByVehicle(element)) then
        local temp = element
            ...
            if (owner > 0) then
                if (faction > 3 or faction < 0) then
                    if (source == towSphere2) then
                        ...
                        if (getElementData(temp, "faction") ~= 76) then
                            if (impounded == 0) then
								...
								local theTeam = exports.pool:getElement("team", 76)
...
function ImpoundFlatbed(element, matchingdimension)
	if (getElementType(element) == "vehicle" and getVehicleOccupant(element) and getElementData(getVehicleOccupant(element),"faction") == 76 and (getElementModel(element) == 578 or getElementModel(element) == 443)) then
					...
					if (getElementData(temp, "faction") ~= 76) then
						if (impounded == 0) then
							...
							if not freeLane or freeLane.veh ~= "0" then
								...
								local factionRank = factionRanks[ getElementData(thePlayer, "factionrank") ] or ""
								exports.global:giveItem(temp, 72, "Towing Notice: Impounded by ".. factionRank .." '".. getPlayerName(thePlayer) .."' at "..datestr)
								exports.announcement:makePlayerNotification(getAccFromCharID(owner), "Your "..exports.global:getVehicleName(temp).." was impounded by Bone County Towing & Recovery", "Visit the T on the map to get it back")
								insertTowRecord(thePlayer, temp)
								local sql = "UPDATE vehicles SET x='" .. mysql:escape_string(freeLane.x) .. "', y='" .. mysql:escape_string(freeLane.y) .."', z='" .. mysql:escape_string(freeLane.z) 
								.. "',   	rotx='" .. mysql:escape_string(freeLane.rx) .. "', roty='" .. mysql:escape_string(freeLane.ry) .. "', rotz='" .. mysql:escape_string(freeLane.rz) 
								.. "',     	currx='" .. mysql:escape_string(freeLane.x) .. "', curry='" .. mysql:escape_string(freeLane.y) .. "', currz='" .. mysql:escape_string(freeLane.z) 
...
function payRelease(vehID)
    if exports.global:takeMoney(source, 250) then
        exports.global:giveMoney(exports.pool:getElement("team", 76), 250)
        exports.mysql:query_free( "INSERT INTO wiretransfers (`from`, `to`, `amount`, `reason`, `type`) VALUES (" .. exports.mysql:escape_string(getElementData(source, "dbid")) .. ", " .. -getElementData(exports.pool:getElement("team", 76), "id") .. ", " .. 250 .. ", 'Vehicle Release', 5)" )
            ...
            if not vehID then
                ...
                if getElementData(vehID, "Impounded") and getElementData(vehID, "Impounded") ~= 0 then
                    if commandName == "unimpound" then
                        if (getElementData(thePlayer,"faction") == 76) then
                    ...
                    elseif commandName == "aunimpound" then
                        if exports.integration:isPlayerTrialAdmin(thePlayer)  then
                            ...
                            if impounder == 1 or impounder == 80 then
...
function updateTowingVehicle(theTruck)
    ...
    if (thePlayer) then
        if (getElementData(thePlayer,"faction") == 76) or (getElementData(thePlayer,"faction") == 762) then
...
function updateCivilianVehicles(theTruck)
    if (isElementWithinColShape(theTruck, towSphere)) then
        ...
        if (dbid >= 0 and faction == -1 and owner < 0) then
            exports.global:giveMoney(exports.pool:getElement("team", 76), 200)
