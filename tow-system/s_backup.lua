...
function destroyBlip()
	for key, value in ipairs(getPlayersInTeam(exports.pool:getElement("team", 76))) do
		outputChatBox("The unit no longer requires assistance. Resume normal patrol", value, 255, 194, 14)
	end
	for k,v in ipairs(getPlayersInTeam(exports.pool:getElement("team", 76))) do
...
function removeBackup(thePlayer, commandName)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or (getElementData(thePlayer, "faction") == 76 and getElementData(thePlayer, "factionleader") == 1) then
...
function backup(thePlayer, commandName)
	...
	--if (factionType==3 or getTeamName(thePlayerTeam) == "Los Santos Towing & Recovery") then--Leaving this in in case of abuse.
		...
		elseif (backupBlip == false) then -- make backup blip
			...
			outputChatBox("You enabled your GPS Beacon for BCT&R.", thePlayer, 0, 255, 0)
		elseif (backupBlip == true) and (backupPlayer==thePlayer) then -- in use by this player
			destroyBlip( )
			outputChatBox("You disabled your GPS Beacon for BCT&R.", thePlayer, 255, 0, 0)
		end
	--end
end
