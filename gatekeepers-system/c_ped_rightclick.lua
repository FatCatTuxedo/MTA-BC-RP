			...
			if (getDistanceBetweenPoints3D(x, y, z, wx, wy, wz)<=3) then
				...
				addEventHandler("onClientGUIClick", bTalkToPed,  function (button, state)
					if(button == "left" and state == "up") then
						...
						local pedType = getElementData(element, "rpp.npc.type")
						...
						elseif (ped=="Jamie Martin") then --Banker ATM Service, MAXIME
						...
						elseif (ped=="Bobby Jones") then --HP impounder / Maxime 
							triggerServerEvent("tow:openImpGui", localPlayer, ped)
						elseif (ped=="Special Agent J. Smith") then --HP release / Maxime
							triggerServerEvent("tow:openReleaseGUI", localPlayer, ped)
						else
