			...
			if not (wMechanic) then
				...
				--if (playerDimension > 0) then
					...
					if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicSix = guiCreateButton( 0.05, y, 0.9, 0.1, "Paintjob - $500", true, wMechanic )
					...
					-- Replace lights
					--if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicNine = guiCreateButton( 0.05, y, 0.9, 0.1, "Replace Lights - $500", true, wMechanic )
					...
					if getVehicleUpgradeOnSlot(vehicle, 8) ~= 0 and faction == 4 then
						...
						if not getElementData(vehicle, "tinted") then
							bMechanicEight = guiCreateButton( 0.05, y, 0.9, 0.1, "Install Tint - $500", true, wMechanic )
							addEventHandler( "onClientGUIClick", bMechanicEight, addTintWindow, false)
							y = y + 0.1
						else
							bMechanicEight = guiCreateButton( 0.05, y, 0.9, 0.1, "Remove Tint - $200", true, wMechanic )
