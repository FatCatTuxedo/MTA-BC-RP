...
function display_vehicle ( id, model, color1, color2, color3, color4, plate, faction, owner, owner_type, impound, stolen, crimes )
	...
	vehW.window = guiCreateWindow( x, y, width, height, tostring(getElementData(localPlayer, "mdc_org")).." - MDC Search - Vehicle: ".. plate, false )
	...
	addEventHandler( 'onClientGUIClick', vehW.window,
		function ()
			if source == vehW.stolen2Button then
				triggerServerEvent( 'mdc-system:updateVehicleStolen', localPlayer, id )
			...
			elseif source == vehW.impoundButton_release then
				if canReleaseLeoImpoundedVehicle[getElementData(localPlayer, "mdc_org")] == impound[2] then
