	...
	local logged = getElementData(thePlayer, "loggedin")
		...
		if not id or not (tonumber(id)) then
			...
			local qWeimy = mysql:query("SELECT `faction_id` FROM `characters` WHERE `id` ='"..issuerCharID.."'")
				...
				if issuerFaction == 50 then
					issuerFaction = "Department of Justice"
				elseif issuerFaction == 1 then
					issuerFaction = "Bone County Sheriff's Office"
				end
					...
					exports.global:giveMoney( getTeamFromName("Fort Carson Municipal Government"), math.ceil(tax*amount) )
...
function issueTicket(thePlayer, theCommand, theVehicle, amount, ...)
	...
	if (logged==1) then
		...
		if (getTeamName(team) == "Department of Justice") or (getTeamName(team) == "Bone County Sheriff's Office") then
			if not (theVehicle) or not (amount) or not (tonumber(theVehicle)) or not (tonumber(amount)) or not (...) then
				...
				triggerEvent('sendAme', thePlayer, "places a ticket on the vehicles windshield")
